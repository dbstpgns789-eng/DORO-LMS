...
✅ **사용자 경험(UX) 중심 개발**  
→ 모든 UI/UX 결정의 최우선 순위를 **사용자 편의성**에 두고 개발했습니다  
→ 실시간 피드백, 직관적인 네비게이션, 시각적 상태 표시 등으로 사용자 혼란 최소화

✅ **API 연동 경험**  
→ 백엔드 개발자와 API 명세를 사전 공유하여 원활한 협업 진행  
→ FormData, CSRF 토큰 처리 등 Django 특성에 맞는 통신 방식 습득

✅ **일관된 디자인 시스템**  
→ CSS Variables를 활용한 통일된 UI로 브랜드 정체성 유지  
→ 컴포넌트 단위 모듈화로 재사용성 확보
...
⚠️ **실제 배포 경험 부재**  
→ 개발 환경에서만 동작하여 **실사용자 피드백 수집 불가**  
→ 성능 최적화, 실제 서버 환경 이슈 등을 경험하지 못한 한계  
**개선 방안**: Vercel/Netlify 등을 활용한 프로토타입 배포, 실사용자 테스트
...
- **즉각적인 피드백**: 이메일 중복 확인, 비밀번호 일치 검증 등 실시간 반응
- **직관적인 네비게이션**: 챗봇 계층 구조, 사이드바 메뉴 등 사용자가 길을 잃지 않도록 설계
- **시각적 상태 표시**: 로딩 스피너, 타이핑 인디케이터, 버튼 상태 변화 등
- **일관된 UI**: CSS Variables로 통일된 디자인 언어 유지
- **반응형 디자인**: 모바일부터 데스크톱까지 모든 디바이스 대응

> **핵심 철학**: 사용자가 시스템을 배우는 것이 아니라, 시스템이 사용자를 이해하도록 설계
